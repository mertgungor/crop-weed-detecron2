...
for folder in d:
    mask_folder = 'PhenoBench-v100/PhenoBench/{}/segmentation_mask'.format(folder)
    output_json_path = 'PhenoBench-v100/PhenoBench/{}/segmentation_mask/via_region_data.json'.format(folder)
        ...
        if filename.endswith(".png"):
            ...
            for contour in contours:
                ...
                if (not len(x_coords) < 5 and not len(y_coords) < 5):
                    ...
                    # Determine the pixel value of the polygon (average value within the contour)
                    polygon_pixel_value = np.mean(mask_image[y_coords, x_coords])

                    # Classify as "weed" (64) or "crop" (255)
                    if polygon_pixel_value == 64:
                        region_attributes = {"name": "weed"}
                    elif polygon_pixel_value == 255:
